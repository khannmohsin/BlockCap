...
class Orchestrator:
    def __init__(self, repo_root: Optional[str]=None, registrar_role: str="Cloud", enforce_signature: bool=True):
        ...
        self.besu_rpc_url = os.getenv("BESU_RPC_URL")   # <— default
    ...
    def _prefunded_index_for_address(self, addr: str) -> Optional[int]:
        ...
        try:
            ...
            for i, acct in enumerate(data.get("prefunded_accounts", [])):
                if acct.get("address", "").lower() == addr_lc:
                    return i
        except Exception:
            pass
        return None
    ...
    def _find_policy_on_chain(self, from_role: str, to_role: str, ops_csv: str, ctx: str):
        ...
        if not np.ok:
            return None
        try:
            next_id = int((np.stdout or "0").strip() or 0) + 1
        except Exception:
            return None
        ...
        for pid in range(1, max(1, next_id)):
            print(f"[find_policy_on_chain] checking pid={pid}")
            try:
                gp = self.get_policy(pid)
                # print(f"[find_policy_on_chain] checking pid={pid}: {gp}")
                ...
                if (gp.get("ctxSchema") or "").lower() != want_ctx:
                    print(f"[find_policy_on_chain] skipping pid={pid} ctxSchema={gp.get('ctxSchema')} != {want_ctx}")
                    continue
                if bool(gp.get("isDeprecated", False)):
                    print(f"[find_policy_on_chain] skipping pid={pid} isDeprecated={gp.get('isDeprecated')}")
                    continue

                print(f"[find_policy_on_chain] matched existing policyId={pid} for {from_role}->{to_role} with ops={ops_csv} and ctx={ctx}")
                return pid
            except Exception:
    ...
    def issue_grant(self, from_sig: str, to_sig: str, policy_id: int, ops_csv: str, expires_at: int, from_idx: Optional[int]=None) -> str:
        ...
        if from_idx is not None:
            env["FROM_IDX"] = str(from_idx)
        r = self._js("issueGrant", from_sig, to_sig, policy_id, ops_csv, expires_at)
    ...
    def get_grant_ex(self, from_sig: str, to_sig: str, policy_id: int) -> Dict[str, Any]:
        ...
        print(f"[get_grant_ex] from_sig={from_sig}, to_sig={to_sig}, pid={policy_id}, ok={r.ok}, stdout={r.stdout!r}, stderr={r.stderr!r}, code={r.code}")
        if not r.ok:
            raise RuntimeError(r.stderr or r.stdout)
        return json.loads(r.stdout)
    ...
    def access_flow(self, from_sig: str, to_sig: str, http_method: str, resource_path: str,
        ...
        print(f"access_flow: from_sig={from_sig}, to_sig={to_sig}, http_method={http_method}, resource_path={resource_path}, expiry_secs={expiry_secs}, allow_delegation={allow_delegation}, delegation_depth={delegation_depth}")
        # Resolve roles
        ...
        try:
            ...
            print(f"[access_flow] get_grant_ex: {gx}")
        except Exception:
            gx = None
