...
for node in os.listdir(NODE_ROOT):
    ...
    for csv_file in ["reg_request.csv", "acc_request.csv"]:
...
marker_legend_handles = [
    mlines.Line2D([], [], color='black', marker='X', linestyle='None',
                  markersize=10, label='Registration Request'),
    mlines.Line2D([], [], color='black', marker='D', linestyle='None',
                  markersize=10, label='Access Request')
]

plt.legend(
    handles=node_legend_handles + marker_legend_handles,
    # title="Nodes",
    loc="lower left",
    frameon=True,
    borderpad=1,
...
plt.tight_layout()
plt.savefig("/Users/khannmohsin/VSCode_Projects/MyDisIoT_Project/Node_measurements/reg_access_request.pdf", format="pdf", bbox_inches="tight")
...
# plt.title("Registration Request Measurements\nMean CPU Time vs Mean Memory Usage")
# plt.grid(True)
# plt.tight_layout()
