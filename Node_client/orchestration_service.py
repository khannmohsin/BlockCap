...
def make_app(repo_root: str | None = None) -> Flask:
    app = Flask(__name__)
    # --- ACK defaults (fix KeyError: 'ACK_GENESIS') ---
    base = Path(repo_root or Path(__file__).parent)
    app.config.setdefault("ACK_ENABLED", True)
    app.config.setdefault("ACK_GENESIS",  base / "genesis/genesis.json")
    app.config.setdefault("ACK_REGISTRY", base / "data/NodeRegistry.json")
    # app.config.setdefault("ACK_PREFUNDED", base / "client_inbox/prefunded_keys.json")  # optional
    app.config.setdefault("ACK_ENODE",    base / "data/enode.txt")
    app.config.setdefault("ACK_SCRIPT",   base / "start_client_services.sh")
    ...
    def acknowledgement():
        print("[ack] Received ACK request")
        # if not app.config.get("ACK_ENABLED", False):
        #     return err("ack_endpoint_disabled", 410)

        try:
            node_id = (request.form.get("node_id") or "").strip()
            enode   = (request.form.get("enode") or "").strip()
            if not node_id or not enode:
                return err("missing node_id or enode", 422)

            # Optional integrity hints (no auth, as requested)
            g_sha = request.form.get("genesis_sha256")
            r_sha = request.form.get("registry_sha256")
            # p_sha = request.form.get("prefunded_sha256")

            # Files from root
            g = request.files.get("genesis_file")
            r = request.files.get("node_registry_file")
            # p = request.files.get("prefunded_keys_file")

            # Ensure parent dirs exist and save files + verify hashes if provided
            app.config["ACK_GENESIS"].parent.mkdir(parents=True, exist_ok=True)
            app.config["ACK_REGISTRY"].parent.mkdir(parents=True, exist_ok=True)
            # app.config["ACK_PREFUNDED"].parent.mkdir(parents=True, exist_ok=True)
            app.config["ACK_ENODE"].parent.mkdir(parents=True, exist_ok=True)

            if g:
                g.save(app.config["ACK_GENESIS"])
                if g_sha:
                    with open(app.config["ACK_GENESIS"], "rb") as fh:
                        if hashlib.sha256(fh.read()).hexdigest() != g_sha:
                            return err("genesis_sha_mismatch", 400)

            if r:
                r.save(app.config["ACK_REGISTRY"])
                if r_sha:
                    with open(app.config["ACK_REGISTRY"], "rb") as fh:
                        if hashlib.sha256(fh.read()).hexdigest() != r_sha:
                            return err("registry_sha_mismatch", 400)

            # if p:
            #     p.save(app.config["ACK_PREFUNDED"])
            #     if p_sha:
            #         with open(app.config["ACK_PREFUNDED"], "rb") as fh:
            #             if hashlib.sha256(fh.read()).hexdigest() != p_sha:
            #                 return err("prefunded_sha_mismatch", 400)

            # Save enode
            app.config["ACK_ENODE"].write_text(enode)

            # # Disable further ACKs (one-shot)
            # app.config["ACK_ENABLED"] = False

            # Start Besu in a new window/shell (cross-platform best effort)
            try:
                script = Path(app.config["ACK_SCRIPT"]).resolve()
                if script.exists():
                    subprocess.Popen(
                        ["bash", str(script), "start-chain-client"],
                        cwd=str(script.parent),
                        start_new_session=True
                    )
                else:
                    print(f"[ack] start script not found: {script}")
            except Exception as e:
                print(f"[ack] start-chain-client spawn error: {e}")

            return ok({
                "message": f"ack stored for node {node_id}",
                "paths": {
                    "genesis": str(app.config["ACK_GENESIS"]),
                    "registry": str(app.config["ACK_REGISTRY"]),
                    # "prefunded": str(app.config["ACK_PREFUNDED"]),
                    "enode": str(app.config["ACK_ENODE"]),
                }
            })
        except Exception as e:
            return err("ack_failed", 500, detail=str(e))
